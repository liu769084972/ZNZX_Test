...
TestDrop::TestDrop(TestUI *testUi, QObject *parent) : QObject(parent), testUI(testUi)
{
...
{
    cameras  = Configure::getInstance()->camerasVector();
    qDebug()<<Common::getInstance()->camera_status_<<"11111";
        ...
        connect(cameras[i].tcpSocekt, &QAbstractSocket::connected, this, [&](){
            ...
            for (int i = 0; i < cameras.size(); ++i) {
                if (cameras[i].tcpSocekt == socket) {
                    ...
                    testUI->labelShowStatus_->setText(deviceState.toList().join("<br>").trimmed());
                    return;
                }
            }
        });
...
void TestDrop::openCamera()
{
    ...
    testUI->labelShowStatus_->clear();
    if(lErr1 == 0 && lErr2 == 0)
    {
        Common::getInstance()->plc_status_ = PLC_ON;
    }else {
