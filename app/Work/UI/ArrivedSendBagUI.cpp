...
ArrivedSendBagUI::ArrivedSendBagUI(QWidget *parent) : QFrame(parent)
{
    setFrameShape(QFrame::Box);
    labelArrived = new QLabel("<font color='red'>到件停止</font>");
    if (Common::getInstance()->comeOrGo_) {
    ...
    arrivedFlag = false;
    sendOrBagFlag = false;
    arrivedRows = Configure::getInstance()->arrivedRow();
    sendOrBagRows = Configure::getInstance()->sendOrBagRow();
    ...
    QBitArray ba = Configure::getInstance()->companySwitch();
