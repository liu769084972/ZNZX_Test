...
SortDataShowUI::SortDataShowUI(QWidget *parent) : QWidget(parent)
{
    int num = Configure::getInstance()->camerasVector().length();
    //设置相机容器个数
...
void SortDataShowUI::setUI()
{
    ...
    for (int i = 0; i < cameras.size(); ++i) {
        ...
        connect(structWidget->buttonConnect, &QPushButton::clicked,
                this, [=]() {
            emit buttonCameraClicked(cameras[i].cameraId, true);});
