...
CalculateSortDataUI::CalculateSortDataUI(QWidget *parent)
    :QFrame(parent)
{

    if (Common::getInstance()->comeOrGo_) {
        baCompany = Configure::getInstance()->companySwitchCome();
    } else {
        baCompany = Configure::getInstance()->companySwitchGo();
    }
    //设置存放标签数组的大小
